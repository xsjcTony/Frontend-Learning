...
- 比如将 `1~100` 分为 `1~33` / `34~66` / `67~100`

特点

- 多用于大型数据库 ( `MySQL` / `Oracle` )

- 支持顺序访问
- 键值和业务相关
- 可能会出现数据倾斜 (某一个分片服务器压力过大)
...
```js
hash(key) % N // N 为分片服务器的数量
```

- 根据 `键值` 计算出 `哈希` 值, 然后取模, 除数为 `分片服务器` 的个数

- 将相同结果的 `键值` 放在相同的 `分片服务器` 中, 实现数据均分
- 比如将 `1~100` 分为`3, 6, 9, ..., 99` / `1, 4, 7, ..., 100` / `2, 5, 8, ..., 98`

问题

- 数据迁移量较大
  - 在后期扩容服务器时, 由于 `分片服务器` 数量的变化, 会导致 `80%` 以上的数据都需要进行迁移
...
- 将整个 `哈希` 值的空间组织成一个 `(0~2)^32` 的 `虚拟圆环`
- 根据 `分片服务器` 的 `哈希` 值, 将服务器配置到 `虚拟圆环` 对应的位置
- 根据需要保存的数据键的 `哈希` 值, 将其放到 `虚拟圆环` 上, 按顺时针方向存入对应的 `分片服务器` 中
- 若数据键的 `哈希` 值超过 `2^32` , 则保存到第一台 `分片服务器` 上

图示

- `分片服务器5` 为扩容的 `分片服务器`

![consistent_hashing.png](D:\xsjcTony\it666\Frontend-Learning\Notes\数据库\Redis\images\consistent_hashing.png)
...
- 在 `Redis Cluster` 中一共有 `16384` 个 `虚拟槽` ( `0 ~ 16383` )
- 这些虚拟槽会被分配给 `分片服务器`
- 存储数据时通过 `CRC16(key) mod 16384` 计算出对应数据键的 `槽值` , 然后保存到对应 `槽值` 的 `分片服务器` 上
- 扩容时, 让现有的 `分片服务器` 转让一些 `虚拟槽` 给新加入的 `分片服务器`

图示

![hash_slot.png](D:\xsjcTony\it666\Frontend-Learning\Notes\数据库\Redis\images\hash_slot.png)
...
- 至少要有 `6` 台服务器 ( `3` 对 `主从结构` )
- 每台 `分片服务器` 都必须是 `主从结构` , 保证了数据的 `安全性` 和 `分流`
- 它自身已经实现保证了 `高可用` , 不需要依赖 `Redis Sentinel`
- 只要给 `主节点` 分配了 `虚拟槽` , 那么它就自动变成了 `分片服务器`
- 所有的 `分片服务器` 之间是能够互相通讯的, 用以保证正确的数据读写

![redis_cluster_structure.png](D:\xsjcTony\it666\Frontend-Learning\Notes\数据库\Redis\images\redis_cluster_structure.png)
...
1. 拷贝至少 `6` 份安装包
2. 修改服务器配置文件 `redis.windows-service.conf`

```js
port 6379 // 端口
daemonize yes // 以守护进程方式运行 (退出cli之后继续运行)
cluster-enabled yes // 表示这是一个 cluster 节点
cluster-config-file nodes-6379.conf // Cluster 信息配置文件, 内容会自动生成
cluster-require-full-coverage no // 是否: 急群众所有节点都能提供服务时, 才向外提供服务 (默认为 yes, 推荐 no)
cluster-node-timeout 15000 // 故障转移或节点超时时间
```
...
4. 启动服务
5. 让服务器之间相互握手
   - 只需要在一个服务器上 `meet` 所有其他服务器, 即可让所有服务器互相认识
...
7. 给 `主节点` 分配 `虚拟槽`
   ...
   - 如果使用 `命令行` 添加, 那么需要一条一条添加

   - 可以写一个 `bat` 脚本自动添加, 不然效率太低

```shell
redis-cli -h host -p port cluster addslots slotNumber
```
...
9. 搭建 `主从结构`
   - `主节点` 的 `node-id` 通过 `cluster nodes` 命令查看 (第6步)

```shell
redis-cli -h slaveHost -p slavePort cluster replicate masterNode-Id
```
...
5. 下载 `redis-trib.rb` 文件 [redis/redis-trib.rb at 3.0 · microsoftarchive/redis](https://github.com/microsoftarchive/redis/blob/3.0/src/redis-trib.rb)

6. 执行该文件, 创建 `Redis Cluster`
   - `--replicas 1` 代表一主一从的关系

```shell
ruby redis-trib.rb create --replicas 1 host1:port1 host2:port2 ...
```
