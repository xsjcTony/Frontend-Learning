...
- `路由服务器` : 一般称作 `mongos` , 用于负责分发用户请求 [mongos — MongoDB Manual](https://docs.mongodb.com/manual/reference/program/mongos/#mongodb-binary-bin.mongos)
...
- 随着时间推移, 某些数据段中保存的数据会越来越多
- 为了保证 `分片平衡` , 当某个数据段数据过多或体积过大的时候, 系统会在下一次操作这个数据段时 (新增 / 更新) 自动将一个大的数据段分裂成多个小的数据段



### 分片平衡

- 当各个 `分片服务器` 上保存的数据段之间数量相差较大时, 会自动触发 `分片服务器` 的数据段迁移
- 在 `MongoDB` 中, 后台会自动运行一个 `集群平衡器` 来负责监视和调整 `分片` 的平衡



### 查询特点

- 如果查询的条件是 `片键` , 那么 `路由服务器` 可以准确的找到保存对应数据的 `分片服务器`
- 如果查询的条件不是 `片键` , 那么
  - `路由服务器` 无法准确的找到保存对应数据的 `分片服务器`
  - 会将请求发送到所有的 `分片服务器` , 然后再将返回的结果整合之后, 再返回给用户



### 集群搭建

[Deploy a Sharded Cluster — MongoDB Manual](https://docs.mongodb.com/manual/tutorial/deploy-shard-cluster/)

1. 搭建 `配置服务器` 复制集
   - 最新版本的 `MongoDB` 要求 `配置服务器` 必须是一个 `复制集`
2. 搭建 `分片服务器` 复制集
   - 用于保存数据的多台电脑
3. 搭建 `路由服务器`
   - 用于建立 `配置服务器` 和 `分片服务器` 之间的关系
